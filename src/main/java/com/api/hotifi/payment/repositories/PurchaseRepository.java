...
public interface PurchaseRepository extends PagingAndSortingRepository<Purchase, Long> {
    ...
    @Modifying
    @Query(value = "UPDATE purchase SET status = status - status % "+ BusinessConfigurations.PAYMENT_METHOD_START_VALUE_CODE +" + ?1, refund_payment_id = ?2, refund_started_at = ?3, refund_transaction_id = ?4 WHERE id = ?5", nativeQuery = true)
